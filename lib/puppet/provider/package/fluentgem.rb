...
Puppet::Type.type(:package).provide :fluentgem, :parent => Puppet::Provider::Package do
  ...
  desc 'Install gem via fluent-gem (included by td-agent)'

  has_feature :versionable

  commands :fluentgem => "/usr/lib64/fluent/ruby/bin/fluent-gem"

  def self.gemlist(options)
    gem_list_command = [command(:fluentgem), "list"]
  ...
  def install
    command = [command(:fluentgem), "install"]
  ...
  def uninstall
    fluentgem "uninstall", "-x", "-a", resource[:name]
  end
